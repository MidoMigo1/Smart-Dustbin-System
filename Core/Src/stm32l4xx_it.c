...
void TIM1_CC_IRQHandler(void)
{
  ...
  /* USER CODE BEGIN TIM1_CC_IRQn 0 */
	...
	// measure the distance between the two edges
		uint32_t dist = (second - first);
		dist = dist/58;
	 dist2 = dist;

	
		 
		if((dist2<=bin_size*0.20))
		{	
			if(flag2==1){
			HAL_GPIO_WritePin(GPIOB, GPIO_PIN_3, GPIO_PIN_SET);
		sprintf(send , "%d THE TRASHBIN IS FULL PLEASE EMPTY \n", dist2);
			HAL_UART_Transmit(&huart2,(uint8_t*) send , 50, 3000);
									}
				else
				{
						flag2=1;
